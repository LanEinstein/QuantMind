...
**review 与随后的修改,一个任务至多一次。** 一轮 review + 一轮修复即止。
**禁止**复验到"无剩余缺陷"、"连续两轮干净才收工"、多轮轮转、为确认收敛再跑一轮。

理由(owner 原话):「有足够的证据表明,多轮 review 会大幅提升 AI 的误判率以及过度纠错。」
真缺陷集中在前一轮,之后是噪声;每次多余的"修复"都在改动已经正确的内容。
与反过度防御第 3 禁(禁用复验循环替代人的判断)同源。

P0/P1 必修;P2/P3 按判断取舍,不必清零;未处置的写进报告即可,不阻塞。docs-only 豁免。
