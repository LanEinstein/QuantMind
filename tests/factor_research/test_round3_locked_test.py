"""Tests for the R3-6 one-shot locked-test verdict logic."""
...
from scripts.factor_research.round3_locked_test import (
...
def test_positive_net_but_loses_to_strong_index_fails_beats_gate() -> None:
    # The round-1/round-2 failure mode: positive net but the index ran harder →
...
def test_frozen_constants_are_filled_and_wellformed() -> None:
    # R3-5 freeze: the pre-commitment constants are filled (not placeholders) and
    # cover exactly the round-3 carry set (eleven + accr; the firewall pins SET).
    from scripts.factor_research.exposure_constraints import CONSTRAINTS
    from scripts.factor_research.round3_locked_test import FROZEN_R3_WEIGHTS_3DP

    assert FROZEN_R3_CONSTRAINT != "PLACEHOLDER_FILLED_IN_R3_5"
    assert FROZEN_R3_CONSTRAINT in CONSTRAINTS
    assert set(FROZEN_R3_WEIGHTS_3DP) == set(R3_CARRY_FACTORS)
    assert "accr" in FROZEN_R3_WEIGHTS_3DP  # the only R3-3 survivor


def _fill_freeze(monkeypatch: pytest.MonkeyPatch) -> None:
    """Simulate the R3-5 freeze so the firewall logic can be exercised filled."""
    import scripts.factor_research.round3_locked_test as m

    monkeypatch.setattr(m, "FROZEN_R3_CONSTRAINT", "constituent_only")
    monkeypatch.setattr(m, "FROZEN_R3_K", 0.05)
    monkeypatch.setattr(m, "FROZEN_R3_A_MAX", 0.01)
    monkeypatch.setattr(m, "FROZEN_R3_NONCONST_CAP", 0.10)
    monkeypatch.setattr(m, "FROZEN_R3_WEIGHTS_3DP", {"ep_ttm": 0.6, "accr": 0.4})
...
def _artifact(**over: object) -> dict[str, object]:
    base: dict[str, object] = {
        ...
        "selected_weights": {"ep_ttm": 0.6, "accr": 0.4},
    }
    base.update(over)
    return base
...
def test_load_frozen_accepts_matching_artifact(
    ...
    assert weights == {"ep_ttm": 0.6, "accr": 0.4}
...
def test_load_frozen_fails_closed_on_drift(
    ...
    cases = [
        ...
        ({"selected_weights": {"ep_ttm": 0.6, "accr": 0.4, "vol_20d": 0.0}}, "set"),
        # a drifted weight value
        ({"selected_weights": {"ep_ttm": 0.7, "accr": 0.4}}, "drifted"),
    ]
...
def test_real_frozen_constants_match_search_result() -> None:
    # The committed FROZEN_R3_* must agree with the on-disk R3-4 search result —
    ...
    path = "data/factor_research/round3_search_result.json"
    if not Path(path).exists():  # artifact is gitignored; skip if absent
        pytest.skip("round3_search_result.json not present")
    constraint, k, a_max, cap, weights = load_frozen_strategy(path)
    assert constraint == FROZEN_R3_CONSTRAINT
    assert set(weights) == set(R3_CARRY_FACTORS)
