"""AF-006 module contract: backend.value_swing import isolation."""
...
import backend.value_swing as pkg

FORBIDDEN_SUBPACKAGES = {"llm", "agents", "mirofish"}
_ROOT = pathlib.Path("backend/value_swing")
...
class TestNoInstructionPlanConstruction:
    @pytest.mark.unit
    def test_overlay_does_not_construct_instruction_plan(self) -> None:
        # The overlay only DECIDES; the builder is the single construction point.
        src = (_ROOT / "swing_overlay.py").read_text(encoding="utf-8")
        assert "InstructionPlan(" not in src
