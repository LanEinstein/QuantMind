"""L-005 module contract: backend.budget_policy import isolation + public API."""
...
import backend.budget_policy as budget_policy

FORBIDDEN_SUBPACKAGES = {"llm", "agents", "mirofish"}
_ROOT = pathlib.Path("backend/budget_policy")
...
class TestPublicAPI:
    ...
    def test_exports_core_symbols(self) -> None:
        for name in (
            ...
            assert hasattr(budget_policy, name), f"budget_policy missing {name}"

    @pytest.mark.unit
    def test_all_is_importable(self) -> None:
        for name in budget_policy.__all__:
            assert hasattr(budget_policy, name), f"__all__ lists missing {name}"
