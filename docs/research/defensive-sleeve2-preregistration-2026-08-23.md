...
## 11. 运行前附录(2026-08-23,codex 唯一一轮 review 驱动;正式运行尚未发生)

codex review(实现审计,3 条 P1)在**正式运行之前**完成;以下修订全部落在实现层,
§5 判据、§6 种子与统计口径、§2 构造一字未动。本附录与修复同 commit 提交,先于正式运行。

### 11.1 调仓日覆盖规则(P1:窗口不许静默缩水)

实测面板前 3 个调仓日(20150202/20150309/20150407)过 gates 后**不存在任何 gpm 有限值的候选**
(2015 年初财报 PIT 覆盖缺失,数据事实)。规则定为:
- 只允许**开头连续的"零候选"日期**被排除(纯覆盖缺失;候选数在 1—9 之间算稀薄,不许排除,直接中止);
- 排除的日期逐一列入结果 JSON(`leading_skipped_dates`),上限 6 个,超限中止;
- 评估开始后的任何日期候选 < 10 → 中止(不存在静默跳过)。
- sleeve1 披露臂对齐到 sleeve2 的实际评估日期集(期收益数组一一对应)。

### 11.2 placebo 轮动语义(P1:硬门对照不得是初始账本买入持有)

随机与 size-matched 臂改为与 sleeve 臂**完全同一套机器**:全候选池打分表
(随机臂 = 逐 (seed,date,code) 确定性 hash 分;size-matched 臂 = 匹配名 1.0/其余 0.0,
平分披露性近似)→ `scores_by_day` + `build_health_overrides` → 同一弱势门轮动。
即:placebo = 同 gates、同容器、同卖出机制、随机的选择信号。seed 不变(20260823)。
**附带披露**:SLV-1 科学门的随机臂(health=None)实为初始账本买入持有;SLV-1 当时
placebo 仅披露不作门,其 PASS 判定不受影响,但该对照的语义弱点在此记录在案。

### 11.3 持仓不相交主张的适用范围(P1:executed ≠ target)

§2 的排除规则(目标书 top-5)不变;"按构造不相交"的主张**限定于目标书**。
事件循环可因健康门保留超出目标窗口的持仓,故两臂**实际执行持仓**可能重叠——
正式运行将从成交记录重建逐日持仓并**披露** executed 重叠
(`executed_holdings_overlap`:重叠天数占比/最大并发重叠/名单),不作断言、不作判据。
