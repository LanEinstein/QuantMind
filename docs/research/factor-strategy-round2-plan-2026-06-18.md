...
### R2-4/R2-5/R2-6 ✅ DONE = 终判 **FAIL**(2026-06-19;feature `0745f2f`/`e9c5f5e`/`69508b5` + freeze `6d71988` + docs `5aa3d3e`;本地未 push)
**owner 决策放宽(`...test-reuse-decision-2026-06-19.md`):测试集可复用 → R2-6 改在既有锁定测试集判定,不等前向。** 全链一次走完:
- **R2-4 暴露约束 + 预声明搜索**:新 `exposure_constraints.py`(constituent_only / size_neutral / capped_nonconstituent,修 R2-3 小盘漂移)穿 `benchmark_relative.build_active_weights`(默认 unconstrained byte-identical)+ frozen `config/research/round2_experiment_manifest.json`(全 DOF,**N=612** 累计 deflation)+ `walk_forward_eval.py`(anchored WF + 组合 purged CPCV)+ `round2_search.py`(612 候选 inner train/val 搜索 → 选唯一 + DSR/PBO/SPA/哨兵/暴露全披露)。
- **终选(R2-4,开发证据≠判定)= `constituent_only`**(真增强指数,size 中性因子不防 universe 漂移→只在 CSI300 成分内 tilt),k=0.10/a_max=0.01,价值/质量复合(ep_ttm0.195/amihud0.160/roe0.140…动量≈0)。**修法成功**:size active −0.67→**−0.11**、TE 2.83%。开发披露**混合**:inner-val IR **1.09**/哨兵过(1.09 vs 0.51)/CPCV 89% 正/SPA-vs-momentum 0.006(显著胜现役动量),**但 DSR=0.056 不过 0.95 + PBO 0.504 + SPA-vs-passive 0.126 不显著** → 诚实=无 deflation-显著 alpha。
- **R2-5 引擎交叉确认 + git 冻结**:`full_engine_crosscheck.py` 成本压力(2× 成本超额 +27.0%→+16.4%,delta −10.6%,**摩擦单调** ✓;rqalpha 全事件循环超范围→UNAVAILABLE 记录,非静默放行)。**git 冻结 commit `6d71988`(读 test 前;`FROZEN_R2_*` 3dp 钉死 + load 复验 fail-closed)**。
- **R2-6 一次性判定**:新 `build_test_panel_r2`(唯一 sanctioned R2 test reader,CLI 仍拒 r2+test)+ `r2_locked_test.py`(冻结-firewall→读 test 一次→四门)。**测试集第 2 次评测(披露)**。结果:净 **+21.58%**/夏普 **1.80**/回撤 **6.11%**/CSI300 +21.84% → **超额 −0.26%** → 四门 **3 过 1 不过 = FAIL**(beats_csi300 以 0.26pp 之差不过)。
- **诚实结论**:① 构造修法成功(round-1 −16.36pp 灾难 → −0.26pp 近平手;测试期 beta≈1〔net active~1e-17〕+ size active −0.064 实证守住)；② 但**无真超额** —— R2-4 的 DSR/SPA 事先精准警告「贴住但不超越」,测试集证实(IR −0.06)；③ **诚实门第二次命中**,没上线微输指数的策略，没偷看测试集凑线，**如实报 FAIL**。④ 现役 momentum0.40 再被 SPA 显著击败仍未获验证。报告 `factor-strategy-round2-result-2026-06-19.md`。
- **门禁**:全链 219 测试绿 + ruff + mypy --strict + redline；`/code-review high`(codex 撞额度回退)全程 **0 正确性 bug + 防火墙 SOUND**,修 6 P2(2 暴露约束 bound 违反 + 2 暴露约束 net-zero/unscored 不变量 + 3 防火墙加固〔weight-set pin / cap 断言 / placeholder fail-closed〕+ 3 质量重构)。
- **下一步 = 下一轮(owner 立项)**:构造已能 beta≈1 跟住指数,缺**正超额 alpha 源** → 深挖基本面 alpha(质量/现金流/预期修正)/ 行业内选股 / 事件因子(需新 PIT);或冻结新策略**等真前向窗口**做第 3 次判定;或与 owner 重定义判据(风险调整相对回报:夏普 1.80/回撤 6.11% 已优)。**本测试集已 2 次评测,慎再迭代。**
