...
- **Commit**: `eb10fc1`
- **Test report**: `docs/reviews/p5b-t03-codex-summary.md` + R1-R7(R1 architecture, R2 UX, R3 testing, R4 perf, R5 security, R6/R7 follow-up verify)
...
#### Phase 5B 出口检查 [🔧 推进中 — harness 就位 / 部署后真值采集]

- **owner_session**: 2026-05-02 main session (claude-opus-4-7-1m + Codex CLI)
- **harness_commit**: 12bac5b
- **wiring_commit**: 51fcfcb (shadow_runner + fund_manager parse_ok 透传 + analysis_scheduler hook + 风控隔离 redline 修复)
...
- **Deferred 到 Phase 5C 单独 task(cross-cutting)**:
  ...
  - 完整 backlog 见 `docs/reviews/phase5b-summary-2026-05-15.md` §6
- **STOP**,等用户授权 7 天部署窗口 + Phase 5C 入口
...
| 2026-05-02 | claude-opus-4-7-1m | 51fcfcb | Phase 5B 出口 wiring 落地(用户授权"启动部署窗口让 shadow_recorder 实跑 7 天"):新增 `backend/services/shadow_runner.py`(env 开关 + sample rate + cost_guard 串行化 + admission control [_MAX_INFLIGHT_SHADOW=4] + 900s 超时 + live `extract_json_from_response` 解析);`config/agent_models.yaml` 新增 `fund_manager_shadow_baseline`(kimi-only,无 routing);`fund_manager._parse_signal` 改返回 `(signal, parse_ok)`,parse_ok 经 `fund_manager_node` → `run_analysis` → `RunCollector.finalize(signal_parse_ok=...)` → `FundManagerRecord.parse_ok` 全链路透传到 routed leg;`analysis_scheduler._run_and_persist_locked` 在 `save_analysis_record` 后 `schedule_shadow_run(services, record_with_signal, self._redis)`(asyncio.create_task,fire-and-forget);`shadow_compare.compute_shadow_report` 增 `parse_failed_pairs` 计数,gate math 仅在 `parse_ok=True` 双 leg 上跑;codex R5 抓出 `backend/risk/engine.py` 经 `backend.data.trading_hours` 间接 `import backend.llm.cost_tracker` 的 redline transitive 违规 → 把 `backend/data/__init__.py` 改空 + 加 `tests/test_risk_isolation_redline.py` subprocess 锁定;1139 pytest passed(+62 net 自 12bac5b),0 failed;7 轮 codex review(R1-R7,14 个发现全部 RESOLVED,无回归)报告在 `docs/reviews/p5b-shadow-r{1..7}-*.md` + `p5b-shadow-codex-summary.md`;operator runbook `docs/reviews/phase5b-shadow-deployment-runbook.md`;**STOP 等用户启动 7 天部署窗口完成真值采集后再判最终出口** |
| 2026-05-05 | claude-opus-4-7-1m | 8543fda | Phase 5B 部署窗口 day-1 阻塞修复(用户复述指令"开启 7 天试运行"):周一 2026-05-04 09:00 slow cron 触发后 5/5 watchlist 全部 `pipeline_timeout` 在 900s,产生 0 条 shadow_decisions。诊断链路:① `tr '\0' '\n' < /proc/${pid}/environ` 确认 4 个 shadow env 已注入 uvicorn;② 日志显示并发分析师阶段 qwen + kimi 同时几秒内 `Connection error.`;③ `socket.getaddrinfo('dashscope.aliyuncs.com')` 返回混合 IPv6 (`2408:400a:...`) + IPv4 记录,但 `ip -6 route show default` 为空,`curl -6` 立即 http=000 失败,`curl -4` 3.5s 通;④ httpx 默认 Happy Eyeballs 让 IPv6 死路阻塞每次连接,叠加 openai SDK 600s 默认 read_timeout × 2 retries 让单卡握手吞光 900s 预算;⑤ `backend/llm/providers.py::create_openai_client` 显式注入 `httpx.AsyncHTTPTransport(local_address="0.0.0.0")` 强 IPv4 + `connect=10s read=120s write=30s pool=10s`,SDK `max_retries=2→1` 让 router-level fallback 提前介入(初始 + 1 retry = 240s 而非 480s);⑥ 新增 `tests/test_llm_router.py::TestCreateOpenaiClient::test_client_binds_ipv4_only` + `::test_client_has_bounded_timeout_and_retries` 锁定 transport pool `_local_address` 与 `max_retries / timeout.connect / timeout.read`;1141 passed, 11 skipped, 0 failed(+2 自 51fcfcb 的 1139);ruff check + format 全绿。修复后端到端验证:`POST /api/analysis/jobs {"stock_code":"600519","max_debate_rounds":1}` 完整跑完 27min(intelligence/bull/bear/risk Kimi 各 timeout 一次 + qwen fallback ~2min,fund_manager qwen triage 通过 confidence=0.65 ≥ 0.6 无需 escalation)→ analysis_records.decision={action:持有, confidence:0.65, parse_ok:True} + trading_signals 持久化。**注意**:27min 超 15min latency gate 是 Kimi 当日 thinking-mode 大 prompt 持续不稳的真实信号;Kimi 健康下应 ~10min 内完成。Shadow_runner 仅 cron 路径触发(`/api/analysis/jobs` 不触发,by design),首次产线 shadow 数据等周三 2026-05-06 09:00 slow cron。memory 写入 `feedback_ipv4_only_egress.md` 永久记录此运维 gotcha;**STOP 等周三起首次 cron 真值数据,再每日跟监控指令做窗口判定;不自动跨阶段** |
