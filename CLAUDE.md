...
**当前状态(2026-07-02,SSoT 重构)**:工程栈**全部完成并冻结**(旧 S/A-X + v2 MVP K-N〔2026-06-01 曾真启〕+ v3 V/W/Y/Z + 存量 O/Q/R/T + 自进化 AA-AE + 价值线 AF 全 done;sim 暂停,运行期激活 owner-gated)。**唯一活跃主线 = alpha-pivot 量化选股 alpha 研究**:owner 2026-06-27 基于真数据重定框定 —— 风险容器**已答**(≤5 集中 + 现金 buffer;分散伤害;EXIT/避顶部/regime overlay 净有害,C1a/B1/B2/QGR-4 四刀全 FAIL 永不再建),**绑定约束 = 选股 alpha 质量**(slot×sizing frontier 全配置 DSR 0.003–0.006 ≪ 0.95)。做法 = fixed prior 复合 spec(反转 0.5 + 分析师 0.25 + 质量 0.25,先验符号 committed,评测前 hash 绝不改)→ 收益盲 power 前置(K=2)→ 可复用竞技场过反过拟合四门(不放宽,账本 N=2417 不清零)→ owner-gated look-once 前向。**执行入口 = `docs/plan.html`(2026-07-02 重构版:DOCS + AP-0..AP-3 + gated FW/SR/SIM + PARKED)**;施工图 = `docs/research/alpha-pivot-implementation-plan-2026-07-02.md`;上位 spec = `docs/research/alpha-pivot-composite-spec-outline-2026-06-27.md`(codex 2 轮收敛)。旧 SSoT(33 phase 全 done)归档 `docs/archive/plan-legacy-2026-07-02.html`。QGR 框定与两层评测(`quant-first-gate-rearch-plan-2026-06-21.md`)仍为上位框架;round-1..4 教训与全部研究原则见下方 blockquote。
...
> **📦 PIT 数据资产已落地(2026-06-22;🚫 禁从头重下载)** —— 全市场 point-in-time 字节存档在 **`data/marketdata_pit/`**(gitignored;content-addressed `payloads/<sha>.bin` + `index.jsonl` + `coverage/`;**~29 GB / ~24k 快照 / 23 端点**)。含 daily/daily_basic/adj_factor/fund_daily(2015-2026)+ fina/income/cashflow/balancesheet + index_weight/index_member_all + report_rc + namechange + **QGR-1 短线/主旋律集(stk_limit/cyq_perf/stk_factor_pro/limit_list_d/suspend_d/forecast_vip/express_vip/ths_index/index_classify)**。**完整端点清单 + 日期范围 + 增量更新协议 + 完整性自检 = `docs/research/data-inventory-marketdata-pit-2026-06-21.md`。后续只增量更新(摄取幂等续传,跳过已存键)+ 更新该清单时间戳,严禁从零重下(stk_factor_pro 单端 32GB 带宽受限 ~4-5h)。**append-only:严禁删/改 payloads。
>
...
| 路径 | 用途 |
|------|------|
| **`docs/plan.html`** | **实施 SSoT(2026-07-02 重构)— alpha-pivot 主线任务清单(DOCS + AP-0..AP-3 + gated FW/SR/SIM + PARKED)+ Session Log(权威下一步指针)+ 研究域门禁 + 维护协议;旧 33 phase 历史归档 `docs/archive/plan-legacy-2026-07-02.html`** |
