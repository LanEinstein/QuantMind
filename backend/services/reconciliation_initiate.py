...
def build_open_reconciliation_ticket(
    *,
    ...
    Returns:
        A frozen OPEN ticket with a placeholder (passed, empty) deviation
        report whose ``ticket_id`` matches the ticket (the model enforces this).
        ``actual_reconciliation_id`` is a ``PENDING-`` placeholder — the resolve
        path keys the user-reported snapshot by ``ticket_id``, not by this field.
    """
